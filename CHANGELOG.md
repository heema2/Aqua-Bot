...
### Added
- **Automod default settings** — global defaults on the automod dashboard: default log channel, ignored channels, and ignored roles that apply to every built-in and custom rule (per-rule ignored lists are added on top)
...
### Improved
- **Auto backup notifications** - each full backup set now sends only **two** developer/channel embeds: one ΓÇ£Backup StartedΓÇ¥ with the full task/file inventory in a code block, and one ΓÇ£Backup Completed/FailedΓÇ¥ with ZIP sizes, upload success/failure, and retention cleanup. Mid-task upload spam removed.
...
### Improved
- **`/help`** - simplified to a fast overview embed with link buttons (dashboard, invite, support, vote); removed the command browser dropdown so replies are instant. Full command lists live on the website / dashboard
...
### Improved
- **Faster simple replies** - configuration and light read commands no longer show a long ΓÇ£thinkingΓÇªΓÇ¥ spinner when they finish quickly; they only defer if work is already close to Discord-s 3-second deadline
...
### Fixed
- **Interaction timeouts** - defer before Discord API / heavy work across moderation, tickets, giveaways, roles, announce, and related slash + button/modal flows so actions no longer finish with ΓÇ£The application did not respondΓÇ¥
...
### Improved
- **Stream alerts embeds** - YouTube, Twitch, and Kick footers now show the creator channel name next to the platform (`YouTube ΓÇó ChannelName ΓÇó Stream Alerts`); dashboard live preview matches
...
### Fixed
- **`/role add` / `/role remove`** - defer the interaction before Discord role API calls so the success reply no longer times out as ΓÇ£The application did not respondΓÇ¥
...
### Fixed
- **YouTube stream alerts** - feed titles/dates are parsed per video entry (no more mismatched ΓÇ£oldΓÇ¥ titles); uploads older than 48h are tracked but never notified; recently seen video IDs are remembered so feed reordering cannot re-alert an older video
...
- **Twitch live** - stricter uptime parsing so junk/rate-limit text is not treated as ΓÇ£liveΓÇ¥
...
### Fixed
- **Supporters Rating** - every vote is stacked into one **final rating** (running average 1.0ΓÇô5.0Γ¡É + ratings received); votes are never overwritten; success/leaderboard show this final score clearly
...
### Fixed
- **Supporters Rating** - ratings now **stack** into a running average (1.0ΓÇô5.0Γ¡É) instead of overwriting the previous vote; leaderboard shows star bar + average + ΓÇ£ratings receivedΓÇ¥; success embeds use the same star visuals
...
### Fixed
- **Automod** - timeouts/mutes are only claimed after Discord actually applies them (no more ΓÇ£mutedΓÇ¥ DMs when the member is still chatting); hierarchy + Timeout Members checks; spam soft-lockout keeps deleting/retrying if timeout fails; dashboard timeout duration uses minutes with clearer setup hints
...
### Added
- **Supporters Rating** - rate supporters with `/rate` (1ΓÇô5Γ¡É, 24h cooldown), `/supporter-leaderboard` for staff, configurable reset interval, top-rated role + announce channel, dashboard Reset now
- **Tickets** - multiple panels, each with its own post channel, roles, limits, and appearance (same pattern as application panels)

### Fixed
- **Dashboard errors** - Discord ΓÇ£Missing Access / Missing Permissions / role hierarchyΓÇ¥ failures now show plain-language guidance instead of raw API codes
...
- **Website** - ΓÇ£Vote for AquaΓÇ¥ link in the site footer (and homepage product links)
- **Dashboard** - floating ΓÇ£Vote for AquaΓÇ¥ badge (bottom-right, 50% opacity) opens the [Top.gg vote page](https://top.gg/bot/1512880652461150289/vote) in a new tab
...
- **Slowmode** - full dashboard rule editor: Discord native (0ΓÇô21600s), shared, and per-user modes, bypass roles, role filters; Discord native applies live on save
...
- **Auto Reaction** - multi-channel keyword rules and announcement packs, editable packs, ΓÇ£react to every messageΓÇ¥ (`matchAll`)
...
### Improved
- **AFK** - dedicated module with dashboard settings (nickname `[AFK]` prefix, mention cooldown, ignore channels/roles), `/afk status`, relative ΓÇ£away sinceΓÇ¥ timestamps, and migration from legacy global AFK data
...
- **Application Panels** - staff review embed title no longer duplicates "Application" (e.g. "New Content Creator Application" instead of "ΓÇªApplication Application")
...
### Fixed
- **Suggestion reaction emojis** - Γ£à and Γ¥î are always used when fields are empty or unset; custom emojis apply when changed
...
- **Giveaways** - winner count quick-picks (1ΓÇô10); optional ping winners above congratulations embed on end/reroll
...
### Added
- **Leveling** - optional ΓÇ£Mention user in channelΓÇ¥ on level-up: pings the member with a custom message (e.g. `{user} Congrats!`) above the embed
...
### Added
- **Giveaways** - Giveaway BoatΓÇôstyle embeds with prize title, extra entries per role, and requirements block
